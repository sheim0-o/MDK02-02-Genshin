...
class Test_test_reviews(unittest.TestCase):
    ...
    def test_corrected_mail(self):
        ...
        for mail in list_mail_cor:
            if bool(re.search(r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+(\.[A-Z|a-z]{2,})+',mail))==False:
                flagCorMail = False
        self.assertTrue(flagCorMail)
        
    def test_uncorrected_mail(self):
        list_mail_uncor = ["sakdja", "", " ", " .", "@", ".@.", "c.~!@mail.ru", "@mail.ru", "@,.ru", 
                           "g@g......"]

        flagUncorMail=False
        for mail in list_mail_uncor:
            if bool(re.search(r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+(\.[A-Z|a-z]{2,})+',mail))==True:
                flagUncorMail = True
